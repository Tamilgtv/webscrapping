...
from . models import db_connect, create_table, Data_Table
from sqlalchemy.orm import sessionmaker

class BhhsPipeline(object):
    def __init__(self):
    ...
    def process_item(self, item, spider):
        if isinstance(item, dict):
            ...
            if item.get('company_name') == "Berkshire Hathaway HomeServices":
                data = Data_Table(item.get('unique_id'),item.get('agent_url'),item.get('agent_dp'),item.get('agent_name'),
